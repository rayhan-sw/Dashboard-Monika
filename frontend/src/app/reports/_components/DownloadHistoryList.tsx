...
const DownloadHistoryList = forwardRef<DownloadHistoryRef, DownloadHistoryListProps>(
  ({ limit = 5 }, ref) => {
    ...
    const loadDownloads = async () => {
      setLoading(true);
      try {
        const response = await reportService.getRecentDownloads(10);
    ...
    return (
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-sm text-orange-500 cursor-pointer hover:underline flex items-center gap-1">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      {formatDate(item.generated_at)} • {item.file_size || "245KB"}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
